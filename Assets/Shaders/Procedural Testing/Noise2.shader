    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float noise(float2 st) 
            {
                ...
                float2 lowerLeftDirection = random2(float2(floor(st.x), floor(st.y))) * 2 - 1;
                float2 lowerRightDirection = random2(float2(ceil(st.x), floor(st.y))) * 2 - 1;
                float2 upperLeftDirection = random2(float2(floor(st.x), ceil(st.y))) * 2 - 1;
                float2 upperRightDirection = random2(float2(ceil(st.x), ceil(st.y))) * 2 - 1;
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                fixed4 UVs = tex2D(_MainTex,i.uv) * gradientColor * heightLine1;
