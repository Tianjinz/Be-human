    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float noise(float2 st) 
            {
                ...
                float2 lowerLeftDirection = rand2dTo2d(float2(floor(st.x), floor(st.y))) * 2 - 1;
                float2 lowerRightDirection = rand2dTo2d(float2(ceil(st.x), floor(st.y))) * 2 - 1;
                float2 upperLeftDirection = rand2dTo2d(float2(floor(st.x), ceil(st.y))) * 2 - 1;
                float2 upperRightDirection = rand2dTo2d(float2(ceil(st.x), ceil(st.y))) * 2 - 1;
                ...
                float upperCells = lerp(upperLeftFunctionValue, upperRightFunctionValue, u.x);
                float lowerCells = lerp(lowerLeftFunctionValue, lowerRightFunctionValue, u.x);

                float genNoise = lerp(upperCells, lowerCells, u.y);
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                //First Noise
                float randomNoise1 = noise(i.worldPos / (_CellSize * _Scale)) + 0.5;
                randomNoise1 = frac(randomNoise1 * 6);


                //Clamped values effect 
                float pixelNoiseChange1 = fwidth(randomNoise1);

                //Invert clamped values to create a "line" from the noise
